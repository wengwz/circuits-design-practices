...
async def run_cocotb_test(dut):
    ...
    basic_tester.clk_settings["wclk"].period = 4
    basic_tester.clk_settings["rclk"].period = 10
...
def test_async_fifo(is_ref, is_wave):
    ...
    parameters = {"DWIDTH":DATA_WIDTH, "AWIDTH":ADDR_WIDTH}
    defines = []
    if is_wave:
...
if __name__ == "__main__":
    ...
    for opt in sys.argv:
        ...
        if opt == '--wave':
            is_wave = True
    test_async_fifo(is_ref, is_wave)
